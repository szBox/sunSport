  ...
  <div class="general">
    ...
    <li v-for="(i,index) in list">
      ...
      <div class="lun" style="margin-top: 0;border-top: 0;margin-left:0;height: 11rem;border-bottom: 1px solid #1F1F21;padding-left: 5%;padding-right: 5%">
        ...
        <div class="xpm" style="width:70%;margin-top: 1rem;text-align: left">得分最高年级<span class="v1">{{i.topgrade}}</span></div>
      </div>
      <div class="cont_b">
...
<script>
  ...
  export default{
    ...
    computed:{
      list(){
        return this.$store.state.d.detail
      }
    },
    mounted(){

      var self=this;
//      alert(self.$store.state.d.week);
      var w=self.$route.params.id;
        ...
        var time=setInterval(function(){
          if($('.f_l').html()!=null){
            for(var i=0;i<len;i++){
              ...
              var name=new Array();
              var score=new Array();
              for(var j=0;j<d.details[i].grade_avg.length;j++){
                name.push(d.details[i].grade_avg[j].name);
                score.push(d.details[i].grade_avg[j].score);
              }
              var x=score;
              var y=d.details[i].school_avg;
              var a1='年级平均分';
              var a2='学校平均分';
